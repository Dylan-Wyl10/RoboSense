"""
Batch runner for the n-weighted coverage objective experiment (YIL-128).
...
`result/ctmResult/logs/ctm_test1/20260817test_nweight/...`.
Bench ctm_gt.npy is COPIED from the 1215test benches (never modified).
...
# ---------------------------------------------------------------------------
# Bench ground-truth provisioning (CTM dynamic demand needs bench/ctm_gt.npy)
# ---------------------------------------------------------------------------

BENCH_SRC_TAG = '1215test'


def provision_bench_gt() -> None:
    """Copy bench/ctm_gt.npy from the 1215test benches into the new OUT_TAG
    tree (same OD demand, so the no-routing ground truth is identical).
    Never overwrites an existing file; never touches the source."""
    for mpr_label in ['2percent', '5percent', '10percent', '10percent_nobgt']:
        sub = f'350_5400s_{mpr_label}_new_normVeh'
        src = os.path.join(PROJECT_ROOT, 'result', 'ctmResult', 'logs', 'ctm_test1',
                           BENCH_SRC_TAG, sub, 'bench', 'ctm_gt.npy')
        dst_dir = os.path.join(PROJECT_ROOT, 'result', 'ctmResult', 'logs', 'ctm_test1',
                               OUT_TAG, sub, 'bench')
        dst = os.path.join(dst_dir, 'ctm_gt.npy')
        if os.path.isfile(dst):
            print(f'  [bench] exists: {dst}')
            continue
        if not os.path.isfile(src):
            raise FileNotFoundError(f'bench source missing: {src}')
        os.makedirs(dst_dir, exist_ok=True)
        shutil.copy2(src, dst)
        print(f'  [bench] copied {src} -> {dst}')
